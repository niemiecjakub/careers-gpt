...
class LatexService:
    ...
    def render_pdf_file(self, render_command, file_name: str, latex_code: str):
        with tempfile.TemporaryDirectory() as tmpdirname:
            tex_path = os.path.join(tmpdirname, f"{file_name}.tex")
            with open(tex_path, "w", encoding="utf-8") as f:
                f.write(latex_code)

            render_command = render_command + [f"{file_name}.tex"] 
            result = subprocess.run(render_command, cwd=tmpdirname, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def render_tex_file(self, rendered_tex: str ,fileName: str):
